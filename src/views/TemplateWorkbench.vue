    ...
    <main class="workspace" :class="{ 'precise-mode': preciseMode }">
      ...
      <section v-else class="pane assistant-pane">
        ...
        <div ref="chatPane" class="scroll chat-scroll">
          ...
          <div v-for="message in messages" :key="message.id" class="bubble" :class="message.kind">{{ message.text }}</div>
          <div v-if="pendingPatch" class="patch"><h3>修改预览{{ pendingPatch.summary ? ` · ${pendingPatch.summary}` : '' }}</h3><div v-if="pendingPatch.operations.length"><div v-for="(operation, index) in pendingPatch.operations" :key="index" class="op"><b>{{ operation.op }}</b> {{ operation.target?.key || operation.target?.section_title || operation.target?.title || '' }}</div></div><div v-else class="notice">AI 没有生成可执行修改</div><div v-if="pendingPatch.questions?.length" class="notice">需要补充：{{ pendingPatch.questions.join('；') }}</div><details><summary>查看完整 Patch JSON</summary><pre>{{ JSON.stringify(pendingPatch, null, 2) }}</pre></details><div v-if="pendingPatch.operations.length" class="patch-actions"><button class="btn primary" :disabled="applying" @click="applyPatch">{{ applying ? '应用中…' : '应用到草稿' }}</button><button class="btn" @click="rejectPatch">拒绝</button></div></div>
        </div>
...
async function ask() { const text = message.value.trim(); if (!text || !draft.value || loading.value || asking.value) return; pushMessage(text, 'user'); message.value = ''; asking.value = true; setStatus('AI 正在分析修改要求…'); pushMessage('AI 正在分析模板结构和修改要求', 'assistant'); try { const response = await askWorkbenchAssistant({ template_id: draft.value.id, message: text, draft: draft.value, selected: buildSelected() }); const result = response.data; pendingPatch.value = result.patch; messages.value[messages.value.length - 1].text = result.message || result.patch.summary || '已生成修改建议'; setStatus('AI 修改建议已生成', 'ok') } catch (error: any) { messages.value[messages.value.length - 1].kind = 'system'; messages.value[messages.value.length - 1].text = `处理失败：${error?.response?.data?.detail || error?.message || '请求失败'}`; setStatus('AI 处理失败', 'err') } finally { asking.value = false; nextTick(scrollChat) } }
async function applyPatch() { if (!draft.value || !pendingPatch.value) return; applying.value = true; try { const response = await applyWorkbenchPatch(draft.value.id, draft.value, pendingPatch.value, draftVersion.value, buildSelected()); draft.value = response.data.draft; draftVersion.value = response.data.draft_version; pendingPatch.value = null; selectionText.value = ''; paintSelectionHighlight(null); pushMessage('修改已应用到当前草稿，尚未写入数据库。请检查后点击“保存模板”。', 'assistant'); if (response.data.validation.errors.length) pushMessage(`校验未通过：${response.data.validation.errors.join('；')}`, 'system') } catch (error: any) { pushMessage(`应用失败：${error?.response?.data?.detail || error?.message || '应用失败'}`, 'system') } finally { applying.value = false } }
function rejectPatch() { pendingPatch.value = null; selectionText.value = ''; paintSelectionHighlight(null) }
function save() { if (dirty.value) void persistSave(); else void persistSave() }
async function persistSave() { if (!draft.value) return; saving.value = true; setStatus('保存中…'); try { const response = await saveWorkbenchTemplate(draft.value.id, draft.value, draftVersion.value); if (!response.data.saved) { pushMessage(`保存前校验未通过：${response.data.validation.errors.join('；')}`, 'system'); setStatus('校验未通过', 'err') } else { dirty.value = false; pushMessage('模板已保存，后续生成报告会使用这版模板。', 'assistant'); setStatus('模板已保存', 'ok') } } catch (error: any) { pushMessage(`保存失败：${error?.response?.data?.detail || error?.message || '保存失败'}`, 'system'); setStatus('保存失败', 'err') } finally { saving.value = false } }
onMounted(load)
</script>
